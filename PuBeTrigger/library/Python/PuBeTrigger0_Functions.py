...
import PuBeTrigger0_RegisterFile
from ctypes import *
import array
import numpy as np
...
def REG_thrs_GET(handle):
    [err, data] = __abstracted_reg_read(PuBeTrigger0_RegisterFile.SCI_REG_thrs, handle)
    return err, data

def REG_thrs_SET(data, handle):
    err = __abstracted_reg_write(data, PuBeTrigger0_RegisterFile.SCI_REG_thrs, handle)
    return err

def REG_inhib_GET(handle):
    [err, data] = __abstracted_reg_read(PuBeTrigger0_RegisterFile.SCI_REG_inhib, handle)
    return err, data

def REG_inhib_SET(data, handle):
    err = __abstracted_reg_write(data, PuBeTrigger0_RegisterFile.SCI_REG_inhib, handle)
    return err

def REG_polarity_GET(handle):
    [err, data] = __abstracted_reg_read(PuBeTrigger0_RegisterFile.SCI_REG_polarity, handle)
    return err, data

def REG_polarity_SET(data, handle):
    err = __abstracted_reg_write(data, PuBeTrigger0_RegisterFile.SCI_REG_polarity, handle)
    return err

def REG_ratereset_GET(handle):
    [err, data] = __abstracted_reg_read(PuBeTrigger0_RegisterFile.SCI_REG_ratereset, handle)
    return err, data

def REG_ratereset_SET(data, handle):
    err = __abstracted_reg_write(data, PuBeTrigger0_RegisterFile.SCI_REG_ratereset, handle)
    return err
...
def RATE_METER_RateMeter_0_GET_DATA(channels, timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_mem_read(channels, PuBeTrigger0_RegisterFile.SCI_REG_RateMeter_0_FIFOADDRESS, timeout_ms, handle)
    return err, data, read_data, valid_data
...
def RATE_METER_RateMeter_0_GET_DATA_COUNTS(channels, timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_mem_read(channels, PuBeTrigger0_RegisterFile.SCI_REG_RateMeter_0_FIFOADDRESS + 512, timeout_ms, handle)
    return err, data, read_data, valid_data
