/********************************************************************
 ...
 Modified by Matt Fritts from below:
...
// Modified by MF: pileup is TRUE if the trigger is too close to the previous one, but not if it's too close to the following one
bool pileup(int triggers_dcrc, int triggers_index){
...
INT basic_trigger (unsigned char trigout[max_tower_number][trigoutbufsize]) {
	...
	for (int dcrc=14;dcrc<MAX_DCRCs;dcrc++){
		...
		for (int i=0; i<primstodo; i++) {
			...
			// If trigger is not piled up then do the following
			if (!piledup) {
